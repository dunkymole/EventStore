...
namespace EventStore.Core.Tests.Services.Transport.Http
{
    ...
    public class when_getting_tcp_stats_from_stat_controller : SpecificationWithMiniNode
    {
        ...
        [Test]
        public void should_return_the_total_number_of_bytes_sent_for_external_connection()
        {
            Assert.Greater(_results.First(r=> r.IsExternalConnection).TotalBytesSent, 0);
        }
        
        [Test]
        public void should_return_the_total_number_of_bytes_received_from_external_connection()
        {
            Assert.Greater(_results.First(r => r.IsExternalConnection).TotalBytesReceived, 0);
        }
